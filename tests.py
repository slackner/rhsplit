...
class RHSplitTests(unittest.TestCase):
    ...
    def test_auto_compression(self):
        with tempfile.TemporaryDirectory() as temp_path:
            ...
            # We use pbkdf2 to generate some deterministic but random looking content.
            # The second part of the file entirely consists of zeros (to see if
            # compression is used)
            content = hashlib.pbkdf2_hmac(
                "sha256", b"PASSWORD", b"SALT", 10, dklen=1024 * 1024 * 10
            )
            ...
            expected = [
                "033c7eb383c672f5a23c64199efff588f532e23c9d5cab8dfde4b095813a94bf",
                "0574d280733582c17775ca52b976b9c85fcfad8c1b88545f67875ade0ec2e212",
                "29606970424baa0d5fbc8e7ae3e182eaae5166d6d387afb1aa771b97e7809c1b",
                "4612fb902faddb62ea7221766297f713d2b1e4690eef543853db2de9139ca94c.gz",
                "64da053347d9606d050e4818ff25fa7e4a78460f247a42c95e5e2b97097e3744",
                "8190655075d5de85e6b061f16a45a659275b14aa07b1dc19704d4ef13e10e011",
                "8a39d2abd3999ab73c34db2476849cddf303ce389b35826850f9a700589b4a90.gz",
                "9c3fb2cd8142593d68615d9fa2475ef61c4e9b538fd9640ab8d410636dc2b07c",
                "a3ed7632d7d307bd6f240acfced9c6ab5125725a65504cd2f1ca5282797d741b",
                "b10b20d787dcede6897ac273f54aa68a664792153348d5c8b170821be191c4e1",
                "c491bfb130da5ce81c918abc4a9cf430cfcb50db02fd7dd322309c4053be42bc",
            ...
            subprocess.check_output(COMMAND + ["-z", data_path], input=content)
            files = set(os.listdir(data_path))
            self.assertEqual(files, set(expected))
    ...
    def test_force_compression(self):
        with tempfile.TemporaryDirectory() as temp_path:
            ...
            # We use pbkdf2 to generate some deterministic but random looking content.
            # The second part of the file entirely consists of zeros.
            content = hashlib.pbkdf2_hmac(
                "sha256", b"PASSWORD", b"SALT", 10, dklen=1024 * 1024 * 10
            )
            content += b"\x00" * 1024 * 1024 * 10
            ...
            expected = [
                "033c7eb383c672f5a23c64199efff588f532e23c9d5cab8dfde4b095813a94bf.gz",
                "0574d280733582c17775ca52b976b9c85fcfad8c1b88545f67875ade0ec2e212.gz",
                "29606970424baa0d5fbc8e7ae3e182eaae5166d6d387afb1aa771b97e7809c1b.gz",
                "4612fb902faddb62ea7221766297f713d2b1e4690eef543853db2de9139ca94c.gz",
                "64da053347d9606d050e4818ff25fa7e4a78460f247a42c95e5e2b97097e3744.gz",
                "8190655075d5de85e6b061f16a45a659275b14aa07b1dc19704d4ef13e10e011.gz",
                "8a39d2abd3999ab73c34db2476849cddf303ce389b35826850f9a700589b4a90.gz",
                "9c3fb2cd8142593d68615d9fa2475ef61c4e9b538fd9640ab8d410636dc2b07c.gz",
                "a3ed7632d7d307bd6f240acfced9c6ab5125725a65504cd2f1ca5282797d741b.gz",
                "b10b20d787dcede6897ac273f54aa68a664792153348d5c8b170821be191c4e1.gz",
                "c491bfb130da5ce81c918abc4a9cf430cfcb50db02fd7dd322309c4053be42bc.gz",
                "cfa406a487faf87c73b646c3dc0d72a08ffbdf32a2e4815791d8a1b1a4ccf5e0.gz",
                "cffa95a3896f2dee7357f9737dcb2130dea79b90729855b557d145553c550b27.gz",
                "e1e8c3092f7527b533e25b2d4f19a6aee806233db3f5faf91baf616ecefbe70c.gz",
                "unpack",
            ]
    ...
    def test_switch_to_compressed(self):
        with tempfile.TemporaryDirectory() as temp_path:
            data_path = os.path.join(temp_path, "data")
            content = os.urandom(1024 * 1024 * 100)
            ...
            # Test inserting content in the middle - but now with compression enabled.
            content = (
                content[: len(content) // 2]
                + os.urandom(4096)
                + content[len(content) // 2 :]
            ...
            # All newly created files should end with the ".gz" file extension.
            for file in (old_files ^ new_files) & new_files:
                self.assertTrue(file.endswith(".gz"))

            # Test what happens if a file is corrupted.
            any_file = next(iter(list((old_files ^ new_files) & new_files)))
            with open(os.path.join(data_path, any_file), "wb+") as fp:
                fp.write(b"GARBAGE")
    ...
    def test_switch_to_uncompressed(self):
        with tempfile.TemporaryDirectory() as temp_path:
            data_path = os.path.join(temp_path, "data")
            content = os.urandom(1024 * 1024 * 100)
            ...
            # Test inserting content in the middle - but now without compression.
            content = (
                content[: len(content) // 2]
                + os.urandom(4096)
                + content[len(content) // 2 :]
            ...
            # All newly created files should not end with the ".gz" file extension.
            for file in (old_files ^ new_files) & new_files:
                self.assertFalse(file.endswith(".gz"))
